...
__global__ void  ComputeVertices(float *vert_tet_weights, int *vert_tet_indices, float *weight_gradients, float *sdf, float *nodes, float *tet_skinning_weights, int *edges_a, int *edges_b, float *params) {
    ...
    float sdf_1 = sdf[sum_1] - level;
    float sdf_2 = sdf[sum_2] - level;
    float eps = 1e-8;
    ...
    if (sdf_1 * sdf_2 < 0.0f) {
        ...
        */
        vert_tet_weights[2*idx] = weight_1;
        vert_tet_weights[2*idx+1] = weight_2;
        vert_tet_indices[2*idx] = sum_1;
        vert_tet_indices[2*idx+1] = sum_2;

        float scalar = 1 / pow(sdf_1 - sdf_2 + eps, 2);
        weight_gradients[4*idx] = scalar*sdf_2;
        weight_gradients[4*idx+1] = -scalar*sdf_2;
        weight_gradients[4*idx+2] = -scalar*sdf_1;
        weight_gradients[4*idx+3] = scalar*sdf_1;
    }
}
""", no_extern_c=True)
