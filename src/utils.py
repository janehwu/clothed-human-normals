...
def loadTSDF_bin(filepath, num_nodes=159645):
        ...
        with open(filepath, "rb") as f:
                try:
                        ...
                        TSDF = np.array(TSDFtup, dtype=np.float32) / convVal
                        print('Original:', np.unique(TSDF))
                        TSDF = TSDF*10
                        #TSDF = (TSDF*10) / 2. + 0.5
                        #print('Rescaled:', np.unique(TSDF))
                        #TSDF[np.where(TSDF < 0, True, False)] = 0
                        #TSDF[np.where(TSDF > 1, True, False)] = 1
                except:
                        print(filepath)
        return TSDF
